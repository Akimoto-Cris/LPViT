...
from vision_transformer import VisionTransformer, _cfg, gumbel_softmax
from sp_vision_transformer import VisionTransformer as VisionTransformer_sp
...
class DistilledVisionTransformer(VisionTransformer):
    ...
    def forward(self, x, tau=-1, number=197):
        x, x_dist = self.forward_features(x, tau, number)
        x = self.head(x)
...
@register_model
def deit_tiny_patch16_224_sp(pretrained=False, **kwargs):
    del kwargs["pretrained_cfg"]
    model = VisionTransformer_sp(
        patch_size=16, embed_dim=192, depth=12, num_heads=3, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
...
@register_model
def deit_tiny_patch16_224_8layer(pretrained=False, **kwargs):
    del kwargs["pretrained_cfg"]
    model = VisionTransformer(
...
@register_model
def deit_small_patch16_224_sp(pretrained=False, **kwargs):
    # del kwargs["pretrained_cfg"]
    model = VisionTransformer_sp(
        patch_size=16, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
...
@register_model
def deit_base_patch16_224_sp(pretrained=False, **kwargs):
    del kwargs["pretrained_cfg"]
    model = VisionTransformer_sp(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
